...
3. **Generate candidates.** A single LLM call, given the company context (including existing initiatives), the peer precedents, the focus area, and the criteria definitions, generates 12 candidate use cases as structured Pydantic output. The prompt includes one or two hand-curated few-shot examples of high-quality outputs for other companies — this is the single highest-impact technique for output quality, anchoring the model on the exact style and structure we want. Negative examples ("here's what bad looks like") are included for each criterion. The prompt explicitly forbids proposals that substantially duplicate the company's existing AI initiatives and requires that at least 3 of the 12 candidates be novel directions rather than direct adaptations of any single precedent.
