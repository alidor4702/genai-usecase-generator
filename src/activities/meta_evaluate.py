...
def _format_use_cases(uses: list[EnrichedUseCase]) -> str:
    ...
    for uc in uses:
        out.append(
            ...
            f"(basis: {uc.time_to_value.basis.value}"
            f"{f', anchored_to: {uc.time_to_value.anchored_to}' if uc.time_to_value.anchored_to else ''}"
            f"{f'; rationale: {uc.time_to_value.rationale}' if uc.time_to_value.rationale else ''})\n"
