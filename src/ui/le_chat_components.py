...
def build_report_component_tree(
    ...
    # the 0.80 bar. The banner differentiates which signal is the issue
    # so a confidence-0.75-but-flagged report doesn't read as contradictory.
    if sales_engineer_ready and (confidence is None or confidence >= 0.80):
        banner_variant = "success"
        banner_title = "Sales-engineer-ready"
        banner_body_text = (
            "Confidence "
            + (f"`{confidence:.2f}` at or above the `0.80` bar. " if confidence else "")
    ...
    elif confidence is not None and confidence >= 0.80 and not sales_engineer_ready:
        banner_variant = "warning"
        banner_title = "Above 0.80 bar, but flagged for strategic revision"
        banner_body_text = (
            f"Confidence `{confidence:.2f}` is at or above the `0.80` numerical bar, "
        ...
        banner_body_text = (
            f"Confidence `{confidence:.2f}` is well below the `0.80` bar. The use "
        ...
        banner_body_text = (
            ...
            + "`0.80` SE-ready bar. The use cases have been through the full "
