...
//
// v7 dropped the emoji-per-step convention in favor of monochrome SVG
// glyphs (see PhaseIcon.tsx). The `tone` Tailwind gradient still drives
// the per-step accent color so each card type reads visually distinct.
...
export const PHASES: { key: PhaseKey; label: string; sub: string }[] = [
  ...
  { key: "review",   label: "Review",   sub: "sales-engineer-ready check" },
];
...
> = {
  ...
  polish: {
    ...
    verb: "Resolving unanchored numbers against the full pool, swapping opaque IDs for markdown links",
  ...
  source_judge: {
    title: "Judging claim ↔ source coherence",
    verb: "Mistral Small reads each (claim, cited URL) pair; rejects sources that just contain related entities without actually supporting the claim",
    tone: "from-yellow-500/30 to-yellow-500/0",
  },
  final_qualify: {
    title: "Qualifying still-unsupported claims",
    verb: "Surgical rewrite: any specific number / named entity the verify chain couldn't anchor becomes qualitative phrasing",
...
export function actorBadge(actor: string): string {
  ...
  if (actor === "tavily" || actor === "tavily.search") return "Tavily web search";
